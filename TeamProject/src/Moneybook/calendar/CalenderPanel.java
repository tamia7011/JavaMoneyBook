		...
		for (int i = 0; i < CAL_HEIGHT; i++) {
			for (int j = 0; j < CAL_WIDTH; j++) {
				...
				File f = new File("MemoData/" + calYear + ((calMonth + 1) < 10 ? "0" : "") + (calMonth + 1)
						+ (dateForm[i][j] < 10 ? "0" : "") + dateForm[i][j] + ".txt");
				if (f.exists()) {
					buttonForm[i][j]
							.setText("<html><b><font color=" + fontColor + ">" + dateForm[i][j] + "</font></b></html>");
				} else
					buttonForm[i][j]
							.setText("<html><font color=" + fontColor + ">" + dateForm[i][j] + "</font></html>");
				...
				if (dateForm[i][j] == 0)
					buttonForm[i][j].setVisible(false);
				else
					buttonForm[i][j].setVisible(true);
			}
		}
	}
